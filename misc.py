...
def hold_gpu(mem_bound=1500, gpus=(0, 1, 2, 3, 4, 5, 6, 7)):
    import sys
    import os
    import subprocess
    import re
    ...
    nGPU = len(utility)
    ideal_gpus = [i for i in range(nGPU) if i in gpus]
    hold_mem = [mem_total - mem_bound - mem[i] for i in ideal_gpus]
    return hold_mem
        
def check_idle_gpu(mem_bound=500, utility_bound=0, gpus=(0, 1, 2, 3, 4, 5, 6, 7)):
    import sys
    import os
    import subprocess
    import re
    ...
    if len(ideal_gpus) > 0:
        msg = 'idle gpus: {}'.format(ideal_gpus)
        print(msg)
    else:
        msg = None
    return msg, ideal_gpus
